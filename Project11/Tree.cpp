...
void Tree::SaveFile(const Node* node) const
{
    const char* path1 = "C:\\task\\point.txt";
     FILE* myFile;
     if (fopen_s(&myFile, path1, "w") != NULL) {
         cout << "Error";
     }
     else {
         if (node != nullptr)
         {
             SaveFile(node->m_left);
             fputs(node->m_english, myFile);
             fputs("   ", myFile);
             fputs(node->m_russian, myFile);
             SaveFile(node->m_right);
         }
         fclose(myFile);
         cout << "Saved";
     }
...
void Tree::Edit(Node* node) {
    cout << endl << "Enter an english word " << endl;
    cin >> node->m_english;

    cout << endl << "Enter a russian word " << endl;
    cin >> node->m_russian;
